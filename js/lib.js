...
dhbgApp.standard.start = function() {
    ...
    $('.measuring-progress').each(function() {
        if (typeof dhbgApp.scorm == 'object') {
            ...
            $this.addClass(type);
            var progress_text = dhbgApp.s('AVANCE');

            switch (type) {
                case 'horizontal':
                    var $box_label = $('<div class="results_value"><label>0</label>%</div>');
                    var $label = $box_label.find('label');
                    var $box_bar = $('<div class="results_level"><div class="progress-bar"></div><div class="progress-slider"></div></div>');
                    var $bar = $box_bar.find('div.progress-bar');
                    var $slider = $box_bar.find('div.progress-slider');
                    $this.append($box_label);
                    $this.append($box_bar);
                    // $this.append($box_slider);
                    $this.append('<div class="progress_text">' + progress_text + '</div>');
                    dhbgApp.loadProgress = function(progress) {
                        $bar.css('width', progress + '%');
                        $slider.css('left', progress-2 + '%');
                        $label.text(progress);
                    };
                    break;
                case 'vertical':
    ...
    $('#pag-esquema map area').on('click', function () {
            var $this = $(this);
            if ($this.attr('data-ref')) {
                dhbgApp.loadPageN($this.attr('data-ref'));
            }
    ...
    $('.box-text').each(function(){
        ...
        if ($this.attr('label')) {
            var $box_title = $('<div class="background-img-text"><div class="title">' + $this.attr('label') + '</div></div>');
    ...
    $('.horizontal-menu').each(function(){
        ...
        var $this = $(this);
        var $chalkboard_items = $('<div class="chalkboard_items board"><div class="chalkboard_pointer_horizontal"></div></div>');
        var $chalkboard_content = $('<div class="chalkboard_content elements"></div>');
        var $chalkboard_pointer_horizontal = $chalkboard_items.find("div.chalkboard_pointer_horizontal");
        ...
        $this.find('>dl').each(function() {
            ...
            var $dt = $('<div class="chalkboard_item button">' + $dl.find('dt').html() + '</div>').on('click', function(){
                ...
                $dd.show();
                var buttonPosition = $item_dt.position();
                var pointerPosition = buttonPosition.left+$item_dt.width()/2+10;
                $chalkboard_pointer_horizontal.css('left', pointerPosition + 'px');
            });
    ...
    $('.vertical-menu').each(function(){
        ...
        var $this = $(this);
        var $chalkboard_items = $('<div class="chalkboard_vertical_items board"><div class="chalkboard_pointer"></div></div>');
        var $chalkboard_content = $('<div class="chalkboard_vertical_content elements"></div>');
        var $chalkboard_pointer = $chalkboard_items.find("div.chalkboard_pointer");
        ...
        $this.find('dl').each(function() {
            ...
            var $dt = $('<div class="chalkboard_vertical_item button">' + $dl.find('dt').html() + '</div>').on('click', function(){
                ...
                $dd.show();
                var buttonPosition = $item_dt.position();
                $chalkboard_pointer.css('top', buttonPosition.top + 'px');
            });
    ...
    $('.ctrl-pagination').each(function() {
        ...
        $items.each(function(){
            ...
            if (data_labelcurrent) {
                $item.append('<div class="label_current">' + label + '/' + $items.length +  '</div>');
            }
...
dhbgApp.standard.load_operations = function() {
                ...
                if(activity.isFullAnswered()){
                    ...
                    if (count_questions > 1) {
                        ...
                        if (weight >= dhbgApp.evaluation.approve_limit) {
                            msg = '<div class="correct"><div class="correct-image"></div>' + dhbgApp.s('all_correct_percent', weight) + '</div>';
                        }
                        else {
                            msg = '<div class="wrong"><div class="wrong-image"></div>' + dhbgApp.s('wrong_percent', (100 - weight)) + '</div>';
                        }
                        $box_end.empty();
    ...
    dhbgApp.actions.activityDroppable = function ($this) {
        ...
        $this.find('[data-target-group]').each(function(){
            ...
            if($item.find('[data-multiple-options]')) {
        ...
        $.each(origins, function(index, origin){
            origin.on('dragstop', function(event, ui){
                ...
                if (end) {
                    ...
                    var count_correct = $this.find('[data-multiple-options]') ? Math.round(activity.countCorrect() / $this.find('[data-target-group="' + $(this).attr('data-group') + '"]').length) : activity.countCorrect();
        ...
        $verify.on('click', function() {
            if (!activity.fullAnswered()){
                ...
                if (weight >= 100) {
                    ...
                    msg = '<div class="wrong"><p>' + feedback + '</p></div>';
